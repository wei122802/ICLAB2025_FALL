...
parameter UPDATE = 3'd5; //update and add drop 
		...
		if(cross_negetive ==1 && cross_zeronum == 0) begin //maybe	
			...
			if(index[0]==2 && cross_negetive>0) begin
				if(update_cnt < Hull_num-drop_temp)
					Hull_new_x[update_cnt] = Hull_x[update_cnt+drop_temp-1] ;
				...
				// 	Hull_new_x [(update_cnt+2-drop_num_comb)%Hull_num] =new_x;
				// else if (!find12)
...
always @(*) begin
	if(current_state ==UPDATE) begin
		if(cross_negetive ==1 && cross_zeronum == 0) begin //maybe	
			...
			if(index[0]==2 && cross_negetive>0) begin
				if(update_cnt < Hull_num-drop_temp)
					Hull_new_y[update_cnt] = Hull_y[update_cnt+drop_temp-1] ;
				else if(update_cnt == Hull_num-drop_temp)
