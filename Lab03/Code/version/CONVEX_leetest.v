...
always @(*) begin
	case (current_state) 
		...
		CROSS         : next_state = cross_done  	? UPDATE		: current_state;
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		update_cnt <= 0;
	// else if (current_state == UPDATE &&!update_done && index[update_done]==1 && index[update_done+1]==2)
	// 	update_cnt <= update_cnt + 2;
...
wire [6:0] drop_temp;
assign drop_temp = (next_state ==UPDATE && cross_negetive>0) ? cross_negetive+cross_zeronum-1 :0;
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		...
		update_new_cnt <= 0;
	else if (current_state==UPDATE && index[update_cnt]==0 &&!update_done)
		update_new_cnt <= update_new_cnt + 1;
	else if (current_state==UPDATE && index[update_cnt]==1 &&!update_done)
		if(index[(update_cnt+1)%Hull_num]==2) update_new_cnt <= update_new_cnt + 2;
		else  update_new_cnt <= update_new_cnt + 1;
	else 
...
always @(*) begin
	if(current_state == RESET)
		...
		if(cross_negetive ==1 && cross_zeronum == 0) begin //maybe	
			...
			if(index[update_cnt%Hull_num]==1 && index[(update_cnt+1)%Hull_num]==2) begin// update_cnt from 1 to 2
				Hull_new_x [(update_new_cnt+1)%Hull_num] =new_x;
				Hull_new_x [update_new_cnt%Hull_num] = Hull_x[update_cnt%Hull_num];
			end
			else
				Hull_new_x [update_new_cnt] = Hull_x[update_cnt];
			// if(index[0]==2 && cross_negetive>0) begin
			// 	if(update_cnt < Hull_num-drop_temp)
			// 		Hull_new_x[update_cnt] = Hull_x[update_cnt+drop_temp] ;
			// 	else if(update_cnt == Hull_num-drop_temp)
			// 		Hull_new_x[update_cnt] = new_x;
			// 	else
			// 		Hull_new_x[update_cnt] = 0;
			// 	if(index[update_cnt%Hull_num]==1 && index[(update_cnt+1)%Hull_num]==2) // update_cnt from 1 to 2
			// 		Hull_new_x [(update_cnt+2-drop_temp)%Hull_num] =new_x;
			// 	else if (!find12)
			// 		Hull_new_x [update_cnt] = Hull_x[update_cnt+(cross_negetive-2)]; //maybe
			// 	else
			// 		Hull_new_x[update_cnt-1] = 0;
			// end
			// else begin
	...
	if(current_state == RESET)
		...
		if(cross_negetive ==1 && cross_zeronum == 0) begin //maybe	
			if (update_cnt < negetive_index) 
		  	...
		  	if(index[update_cnt%Hull_num]==1 && index[(update_cnt+1)%Hull_num]==2) begin// update_cnt from 1 to 2
				Hull_new_y [(update_new_cnt+1)%Hull_num] =new_y;
				Hull_new_y [update_new_cnt%Hull_num] = Hull_y[update_cnt%Hull_num];
			end
			else
...
// 			if(index[0]==2 && cross_negetive>0) begin
// 				// if(update_cnt < Hull_num-drop_temp)
// 				// 	Hull_new_y[update_cnt] = Hull_y[update_cnt+drop_temp] ;
// 				// else if(update_cnt == Hull_num-drop_temp)
// 				// 	Hull_new_y[update_cnt] = new_y;
// 				// else
// 				// 	Hull_new_y[update_cnt] = 0;
// 				if(index[update_cnt%Hull_num]==1 && index[(update_cnt+1)%Hull_num]==2)
// 					Hull_new_y [(update_cnt+2-drop_temp)%Hull_num] =new_y;
// 				else if (!find12)
// 					Hull_new_y [update_cnt] = Hull_y[update_cnt+cross_negetive-2]; //maybe
// 				// else
// 				// 	Hull_new_y[update_cnt-1] = 0;
// 			end
// 			else begin
// 				if(index[update_cnt]==1 &&index[update_cnt+1]==2)
// 					Hull_new_y [update_cnt+1] =new_y;
