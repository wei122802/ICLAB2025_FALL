...
always @(*) begin
	...
	// 								new_x, new_y
	// 	);
	if((next_state==CROSS_PRODUCT||next_state == DEAL_TRIANGLE) && cross_cnt < Hull_num)
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		Hull_num <=0;
	else if (current_state == DEAL_TRIANGLE)
		Hull_num <=3;
...
always @(posedge clk or negedge rst_n) begin
	...
	else if (in_valid_comb) negetive_index <= 0;
...
always @(*) begin
	...
	case (cross_negetive)
		...
		2: begin
			case (cross_zeronum)
				0 : begin
					Hull_new_x[needdrop_index] = new_x;
				end
				// default: 
			endcase
		end
		default : begin
			if(index[0]==2) begin
				for (k = 0; k < 127; k = k + 1) begin
					...
					else if (k <= Hull_num-cross_negetive+1) begin
						Hull_new_x[k] = Hull_x[k+drop_num_comb-2];
					end
					else if( k > Hull_num-cross_negetive+1)
						Hull_new_x[k] = 0 ;
					else begin
						Hull_new_x[k] = Hull_x[k];
					end
				...
				// Hull_new_x[0] = new_x;
				// Hull_new_x[1] = Hull_x[5];
				// Hull_new_x[2] = Hull_x[6];
				// Hull_new_x[3] = Hull_x[7];
				// Hull_new_x[4] = Hull_x[8];
				// Hull_new_x[5] = Hull_x[9];
				// Hull_new_x[6] = Hull_x[10];
				// for (i = 7 ; i <=128 ; i = i +1) Hull_new_x[i] =0;
			end 
			...
			if(index[0]==2) begin
				for (k = 0; k < 127; k = k + 1) begin
					if (k==0) begin
						...
						Hull_new_y[k] = Hull_y[k+drop_num_comb-2];
					end
					else if( k > Hull_num-cross_negetive+1)
						Hull_new_y[k] = 0 ;
					else begin
...
always @(posedge clk or negedge rst_n) begin
	...
	else if(next_state==CROSS_PRODUCT && (cross_[23] ==1)) begin
