...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		point_most <=0;
	else if(in_valid && current_state==WAIT_FIRST)
		point_most <= pt_num;
	else
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)
		...
		if (index[update_cnt]!=2)
			if (index[update_cnt%Hull_num]==1 && index[(update_cnt+1)%Hull_num]==2 ) update_new_cnt <= update_new_cnt + 2;
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) for(i=0 ; i<=128 ; i=i+1) Hull_new_x[i] <=0;
	else if (current_state == RESET) for(i=0 ; i<=128 ; i=i+1) Hull_new_x[i] <=0;
		...
		if(cross_negetive ==1 && cross_zeronum == 0) begin //maybe	
			...
			if(index[0]==2 && cross_negetive>0) begin
				...
				Hull_new_x [Hull_num-drop_temp] <= new_x;
			end
			else begin
...
always @(posedge clk or negedge rst_n) begin
	...
	else if (current_state == RESET) for(i=0 ; i<=128 ; i=i+1) Hull_new_y[i] <=0;
	else if(current_state ==UPDATE) begin
		for(i=0 ; i<=128 ; i=i+1) 
			if(i>Hull_num)
				Hull_new_y[i] <=0;
			...
			if(index[0]==2 && cross_negetive>0) begin
				...
				Hull_new_y [Hull_num-drop_temp] <= new_y;
			end
			else begin
				if(index[update_cnt]==1 &&index[update_cnt+1]==2)
					Hull_new_y [update_cnt+1] <=new_y;
...
// Cycle: 40.00
// Area: 1173743.545186
// Performance: 46949741.80744000

// Cycle: 35.00
// Area: 1172170.158006
// Performance: 41025955.53021000

// Cycle: 25.00
// Area: 1183519.834903
// Performance: 29587995.87257500

// Cycle: 15.00
// Area: 1189454.132239
// Performance: 17841811.98358500

// Cycle: 12.00
// Area: 1285144.678332
// Performance: 15421736.13998400
...
//v2 reduce cross bits (24 -> 22)
// Cycle: 11.00
// Area: 692240.482754
// Performance: 7614645.31029400 // 03 FAIL

//v3 let cross to change 2bits NO.2