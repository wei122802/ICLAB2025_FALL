...
logic [16:0] hp_new;
logic exceed_exp, exceed_mp, less_exp ,less_attack, less_defense ,exceed_hp ,less_mp ,exceed_defense;
    ...
    case (current_state)
        ...
        CALU : begin // Opt
            case(active_state_reg)
                ...
                skill_active:   next_state = (mp_flag) ? OUT : (calu_cnt > 3) ? WAIT_DRAM :  CALU;  //3
...
// assign delta_min1 = A[2] - A[0];
// assign delta_min2 = A[3] - A[1];
...
always_ff @ (posedge clk or negedge inf.rst_n) begin
    if (!inf.rst_n) begin
        ...
        case (type_reg) 
            ...
            Type_B : begin
                // delta[calu_cnt] <= (min1 == calu_cnt)      ? (A[2] - A[0]) : (min2 == calu_cnt)     ? (A[3] - A[1]) : 0;
                ...
                delta[3] <= (min1 == DEFENSE) ? (A[2] - A[0]) : (min2 == DEFENSE)? (A[3] - A[1]) : 0; // calu_cnt 1~
            end
            Type_C : begin
...
always_ff @ (posedge clk or negedge inf.rst_n) begin
    if (!inf.rst_n) begin
        ...
        case (mode_reg) 
            Easy   : begin
                if(calu_cnt>2)
                    delta_final[calu_cnt-3] <= delta[calu_cnt-3] - (delta[calu_cnt-3] >> 2) ;
...
always_ff @(posedge clk or negedge inf.rst_n) begin
    if (!inf.rst_n)   mp_remain <= '0;
        ...
        case(calu_cnt)
            ...
            default: mp_remain <= (mp_remain >= mp_skill_reg[calu_cnt]) ? mp_remain - mp_skill_reg[calu_cnt] : mp_remain ;
        endcase 
    end
...
//===============================================date_diff_calculator add 1 level reg 
//best
// Cycle: 3.80
// Area: 135738.187702
