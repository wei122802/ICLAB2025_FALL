        ...
        for (c1=0; c1<9; c1=c1+1) begin
            for (c2=c1+1; c2<9; c2=c2+1) begin
                if (board[s][c1] != 0 && board[s][c1] == board[s][c2]) begin
                    rowdead = (current_state==BACKTRACK || current_state==BACKTRACK_2) ;
                end
            end
        end
    end
...
always @(posedge clk or negedge rst_n) begin
	...
	else if(next_state == BACKTRACK || next_state == BACKTRACK_2) 
		for(n=0 ; n<9 ; n=n+1)
			for(r=0 ; r<9 ; r=r+1)
				if(backtrackstart)
...
always @(*) begin
	case(current_state)
		...
		SOLVE_UPDATE: begin
			...
			else if (backtrackstart ) next_state = BACKTRACK;
			else 		next_state = SOLVE_UPDATE ;
		end
		BACKTRACK : 
			if(has_contradiction) next_state = BACKTRACK_2 ;
			else if (done)	next_state = OUTPUT ; 
			else next_state = BACKTRACK ;
		BACKTRACK_2 :
			if(done) next_state = OUTPUT ; 
			else next_state = BACKTRACK_2;
		OUTPUT: begin
...
always @(posedge clk or negedge rst_n) begin
	if (!rst_n) begin
		...
		else if (current_state==BACKTRACK_2) begin
			for( i=0 ; i<9 ; i=i+1 )
				for( j=0 ; j<9 ; j=j+1 ) begin
					if(has_contradiction)
						if(i == bt_row && j== bt_col)
							board[bt_row][bt_col] <= try_value2;
						else
							board[i][j] <= backtrackboard[i][j];
					else if (naked_single[i][j]) 
						board[i][j] <= naked_value[i][j];
					else if (hidden_single[i][j]) begin
						board[i][j] <= hidden_value[i][j];
					end 
