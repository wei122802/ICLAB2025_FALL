...
parameter BACKTRACK    = 3'd1 ;
parameter SOLVE_UPDATE = 3'd2 ;
parameter OUTPUT       = 3'd3 ;
parameter BACKTRACK_2  = 3'd4 ;
...
assign has_contradiction = rowdead ;
integer s,c1,c2;
wire backtrackupdate;
reg [3:0] try_value2;
// assign backtrackupdate = (try_value!=0 && try_value2!=0 && current_state !=SOLVE_UPDATE);
...
assign backtrackupdate = ( board[bt_count/9][bt_count%9] == 0 && possible_count[bt_count/9][bt_count%9] == 2 );

always @(posedge clk or negedge rst_n) begin
	if(!rst_n)	
		bt_row <= 4'd0 ;
	else if (current_state==BACKTRACK && backtrackupdate)
		bt_row <= bt_count/9 ;
	else 
		bt_row <= bt_row ;
end
...
always @(*) begin
		done =  ~has_contradiction  && board[0][0] != 0 && board[0][1] != 0 && board[0][2] != 0 && board[0][3] != 0 && board[0][4] != 0 && board[0][5] != 0 && board[0][6] != 0 && board[0][7] != 0 && board[0][8] != 0
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n)begin
		...
		case (1)
			(possible_values[bt_row][bt_col][1])  : begin 
				try_value <= 4'd1 ; 
				case (1)
					(possible_values[bt_row][bt_col][2])  : try_value2 <= 4'd2 ;
					(possible_values[bt_row][bt_col][3])  : try_value2 <= 4'd3 ;
					(possible_values[bt_row][bt_col][4])  : try_value2 <= 4'd4 ;
					(possible_values[bt_row][bt_col][5])  : try_value2 <= 4'd5 ;
					(possible_values[bt_row][bt_col][6])  : try_value2 <= 4'd6 ;
					(possible_values[bt_row][bt_col][7])  : try_value2 <= 4'd7 ;
					(possible_values[bt_row][bt_col][8])  : try_value2 <= 4'd8 ;
					(possible_values[bt_row][bt_col][9])  : try_value2 <= 4'd9 ;
					default: try_value2 <= try_value2 ;
				endcase
				end
			(possible_values[bt_row][bt_col][2])  : begin 
				try_value <= 4'd2 ; 
				case (1)
					(possible_values[bt_row][bt_col][3])  : try_value2 <= 4'd3 ;
					(possible_values[bt_row][bt_col][4])  : try_value2 <= 4'd4 ;
			...
			(possible_values[bt_row][bt_col][3])  : begin
				try_value <= 4'd3 ;
				case (1)
					(possible_values[bt_row][bt_col][4])  : try_value2 <= 4'd4 ;
					(possible_values[bt_row][bt_col][5])  : try_value2 <= 4'd5 ;
			...
			(possible_values[bt_row][bt_col][4])  : begin
			try_value <= 4'd4 ;
				case (1)
					(possible_values[bt_row][bt_col][5])  : try_value2 <= 4'd5 ;
					(possible_values[bt_row][bt_col][6])  : try_value2 <= 4'd6 ;
			...
			(possible_values[bt_row][bt_col][5])  : begin 
			try_value <= 4'd5 ; 
				case (1)
					(possible_values[bt_row][bt_col][6])  : try_value2 <= 4'd6 ;
					(possible_values[bt_row][bt_col][7])  : try_value2 <= 4'd7 ;
			...
			(possible_values[bt_row][bt_col][6])  : begin
			try_value <= 4'd6 ;
				case (1)
					(possible_values[bt_row][bt_col][7])  : try_value2 <= 4'd7 ;
					(possible_values[bt_row][bt_col][8])  : try_value2 <= 4'd8 ;
			...
			(possible_values[bt_row][bt_col][7])  : begin 
			try_value <= 4'd7 ;
				case (1)
					(possible_values[bt_row][bt_col][8])  : try_value2 <= 4'd8 ;
					(possible_values[bt_row][bt_col][9])  : try_value2 <= 4'd9 ;
			...
			(possible_values[bt_row][bt_col][8])  : begin
			try_value <= 4'd8 ; 
			try_value2<= 4'd9 ;
			end
...
always @(posedge clk or negedge rst_n) begin
	...
	else if (current_state == BACKTRACK) num_filled <=backtrackupdate;
	else num_filled <= num_filled_comb ; 
end

always @(*) begin
	if(next_state==SOLVE_UPDATE)
...
// reg [6:0] remain_emptynum; 
// reg [6:0] remain_emptynum_temp;

reg [6:0] remain_emptynum_temp;

always @(posedge clk or negedge rst_n) begin
	if(!rst_n)	remain_emptynum_temp <=0;
	else if (backtrackstart) remain_emptynum_temp <= remain_emptynum ;
	else if (current_state==OUTPUT) remain_emptynum_temp <= 0;
	else remain_emptynum_temp <= remain_emptynum_temp ;
end

// always @(*) begin
// 	remain_emptynum = (board[0][0]==0) + (board[0][1]==0) + (board[0][1]==0) + 
...
always @(posedge clk or negedge rst_n) begin
	if (!rst_n) 	remain_emptynum <= 7'd0 ; 
		...
		else if (has_contradiction && current_state!=INPUT)			  remain_emptynum <= remain_emptynum_temp ;
...
always @(*) begin
	case(current_state)
		...
		BACKTRACK : 
			if (backtrackupdate) next_state = SOLVE_UPDATE ;
			...
			for( i=0 ; i<9 ; i=i+1 )
				for( j=0 ; j<9 ; j=j+1 ) begin
					if(has_contradiction)
						if(i == bt_row && j==bt_col )
							board[i][j] <= try_value2;
						else
