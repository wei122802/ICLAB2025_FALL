...
module Packet_allocator (
    ...
    // Fallback allocation function
    function [1:0] fallback_allocate;
        input [1:0] pivot;
        input ch0_avail, ch1_avail, ch2_avail;
        begin
            case (pivot)
    ...
    function [1:0] update_pivot;
        ...
        begin
            if (success) begin
                ...
                // Failure: next_pivot = (pivot + 2) % 3
                case (current_pivot)
                    2'b00: update_pivot = 2'b10;
                    2'b01: update_pivot = 2'b00;
                    2'b10: update_pivot = 2'b01;
        ...
        if (!req_valid || prefer_ch > 2'b10) begin
            ...
            case (prefer_ch)
                2'b00: begin // Prefer channel 0
                    if (ch0_not_full) begin
                        // Allocate to preferred channel
                        allocation = 2'b00;
                        ch0_used_next = ch0_used + 1;
                        capacity_full_next[0] = (ch0_capacity == ch0_used_next);
                        // Pivot state unchanged for preferred allocation
                    end else begin
                        // Preferred channel full, mark it
                        capacity_full_next[0] = 1'b1;
                        
                        // Fallback allocation
                        if (!pivot_initialized) begin
                            // First fallback - initialize pivot
                            pivot_init_next = 1'b1;
                            allocation = fallback_allocate(prefer_ch, ch0_not_full, ch1_not_full, ch2_not_full);
                            next_pivot = update_pivot(prefer_ch, allocation != 2'b11);
                        end else begin
                            // Use existing pivot
                            allocation = fallback_allocate(current_pivot, ch0_not_full, ch1_not_full, ch2_not_full);
                            next_pivot = update_pivot(current_pivot, allocation != 2'b11);
                        end
                        
                        // Update usage and capacity based on allocation
                        case (allocation)
                            2'b00: begin
                                ch0_used_next = ch0_used + 1;
                                capacity_full_next[0] = (ch0_capacity <= ch0_used + 1);
                            end
                            2'b01: begin
                                ch1_used_next = ch1_used + 1;
                                capacity_full_next[1] = (ch1_capacity <= ch1_used + 1);
                            end
                            2'b10: begin
                                ch2_used_next = ch2_used + 1;
                                capacity_full_next[2] = (ch2_capacity <= ch2_used + 1);
                            end
                            default: ; // unallocated
                        endcase
                    end
                ...
                2'b01: begin // Prefer channel 1
                    if (ch1_not_full) begin
                        allocation = 2'b01;
                        ch1_used_next = ch1_used + 1;
                        capacity_full_next[1] = (ch1_capacity == ch1_used_next);
                    end else begin
                        capacity_full_next[1] = 1'b1;
                ...
                2'b10: begin // Prefer channel 2
                    if (ch2_not_full) begin
                        allocation = 2'b10;
                        ch2_used_next = ch2_used + 1;
                        capacity_full_next[2] = (ch2_capacity == ch2_used_next);
                    end else begin
                        capacity_full_next[2] = 1'b1;
...
compare2 COM_5 (.in1(First2) , .in2(First6) , .out1(Second1) , .out2(Second2) );
compare2 COM_6 (.in1(First1) , .in2(First5) , .out1(Second3) , .out2(Second4) );
compare2 COM_7 (.in1(First4) , .in2(First8) , .out1(Second7) , .out2(Second8) );
compare2 COM_8 (.in1(First3) , .in2(First7) , .out1(Second5) , .out2(Second6) );

wire signed [13:0]Third1,Third2,Third3,Third4,Third5,Third6,Third7,Third8;
compare2 COM_9  (.in1(Second3) , .in2(Second5) , .out1(Third1) , .out2(Third2) );
compare2 COM_10 (.in1(Second1) , .in2(Second7) , .out1(Third3) , .out2(Third4) );
compare2 COM_11 (.in1(Second4) , .in2(Second6) , .out1(Third5) , .out2(Third6) );
compare2 COM_12 (.in1(Second2) , .in2(Second8) , .out1(Third7) , .out2(Third8) );

wire signed [13:0]Forth1,Forth2,Forth3,Forth4;
compare2 COM_13 (.in1(Third3) , .in2(Third5) , .out1(Forth1) , .out2(Forth2) );
compare2 COM_14 (.in1(Third4) , .in2(Third6) , .out1(Forth3) , .out2(Forth4) );

wire signed [13:0]Fifth1,Fifth2,Fifth3,Fifth4;
compare2 COM_15 (.in1(Third7) , .in2(Forth3) , .out1(Fifth1) , .out2(Fifth2) );
compare2 COM_16 (.in1(Third2) , .in2(Forth2) , .out1(Fifth3) , .out2(Fifth4) );

wire signed [13:0]Sixth1,Sixth2,Sixth3,Sixth4,Sixth5,Sixth6;

compare2 COM_17 (.in1(Fifth1) , .in2(Fifth4) , .out1(Sixth1) , .out2(Sixth2) );
compare2 COM_18 (.in1(Fifth3) , .in2(Forth1) , .out1(Sixth3) , .out2(Sixth4) );
compare2 COM_19 (.in1(Fifth2) , .in2(Forth4) , .out1(Sixth5) , .out2(Sixth6) );

assign out4 = Sixth1[8:0];
assign out5 = Sixth2[8:0];
assign out2 = Sixth3[8:0];
assign out3 = Sixth4[8:0];
assign out6 = Sixth5[8:0];
assign out7 = Sixth6[8:0];
assign out1 = Third1[8:0];
assign out8 = Third8[8:0];
...
// 32.5 violation
//=================================================================
//v5 25compare -> 19compare2 
// Cycle: 34
// Area: 211376.088853

//=================================================================
//v6 reduce merge sort from 13 bits to 8bits
// Cycle: 34
// Area: 202418.093651
...
// change rebalance to grant_channel

//Cycle: 33
// Area: 218933.669620
// Performance: 238418766.216180

// Cycle: 32.7
// Area: 225270.461651
// Performance: 240879451.93879779

// Cycle: 32.5
// Area: 227662.143102
// Performance: 240468138.65148750

// Cycle: 32.3
// Area:  229139.064777
// Performance: 245614035.61403561

// 32.1 violate!!!!!

//v6 alloction optimizate
// Cycle: 33.8
// Area: 201579.840832
// Performance: 230292873.36011008
