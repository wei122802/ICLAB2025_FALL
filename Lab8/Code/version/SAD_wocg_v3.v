...
wire calu_P = (input_cnt >= 186) || input_finish_flag || start_output ;
...
always @ (posedge clk or negedge rst_n) begin
    ...
    else if(input_cnt == 8'd191)  input_finish_flag <= 1'b1;
    else input_finish_flag <= 0;
end

always @(posedge clk or negedge rst_n) begin
    if(!rst_n)  cal_cnt <= 4'd0;
    else if(input_finish_flag || ((cal_cnt>0) && (cal_cnt<=2)))  cal_cnt <= cal_cnt + 4'd1;
    else cal_cnt <= 4'd0;
end
...
wire [1:0] Det_cnt = input_cnt - 186;
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        out_data <= P[output_cnt/8][output_cnt%8] * Det_y;
    end
    else begin
        out_data <= 92'd0;
    end
