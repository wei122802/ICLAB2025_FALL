...
generate
    case(IP_WIDTH)
        2 : begin
            assign OUT_WINNER[0] = (score[0] >= score[1]) ;
            assign OUT_WINNER[1] = (score[1] >= score[0]) ;
        end
        3 : begin
            assign OUT_WINNER[0] = (score[0] >= score[1]) && (score[0] >= score[2]);
            assign OUT_WINNER[1] = (score[1] >= score[0]) && (score[1] >= score[2]);
            assign OUT_WINNER[2] = (score[2] >= score[0]) && (score[2] >= score[1]);
        end
        4 : begin
            for(i=0;i<4;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]);
            end
        end
        5 : begin
            for(i=0;i<5;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]) && (score[i] >= score[4]);
            end
        end
        6 : begin
            for(i=0;i<6;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]) && (score[i] >= score[4]) && (score[i] >= score[5]);
            end
        end
        7 : begin
            for(i=0;i<7;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]) && (score[i] >= score[4]) && (score[i] >= score[5]) && (score[i] >= score[6]);
            end
        end
        8 : begin
            for(i=0;i<8;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]) && (score[i] >= score[4]) && (score[i] >= score[5]) && (score[i] >= score[6]) && (score[i] >= score[7]);
            end
        end
        default: begin
            for(i=0;i<9;i=i+1) begin 
                assign OUT_WINNER[i] = (score[i] >= score[0]) && (score[i] >= score[1]) && (score[i] >= score[2]) && (score[i] >= score[3]) && (score[i] >= score[4]) && (score[i] >= score[5]) && (score[i] >= score[6]) && (score[i] >= score[7]) && (score[i] >= score[8]);
            end
        end
    endcase
endgenerate
...
// generate
//     case(IP_WIDTH)
//         2: compare compare2 (.in1(score[0]), .in2(score[1]), .max(maxscore));
//         3: begin
//             compare compare3_1 (.in1(score[0]), .in2(score[1]), .max(max_temp));
//             compare compare3_2 (.in1(max_temp), .in2(score[2]), .max(maxscore));
//         end
...
//             compare compare5 (.in1(max_temp), .in2(score[4]), .max(maxscore));
//         end
//         6: begin
//             findmax4 findmax6_1 (.in1(score[0]), .in2(score[1]), .in3(score[2]), .in4(score[3]) , .maxvalue(max_temp));
//             compare compare6 (.in1(score[4]), .in2(score[5]), .max(max_temp1));
//             compare compare6_1 (.in1(max_temp), .in2(max_temp1), .max(maxscore));
//         end
//         7: begin
...
//             compare compare7 (.in1(max_temp), .in2(max_temp1), .max(maxscore));
//         end
//         8: begin
...
//             compare compare8 (.in1(max_temp), .in2(max_temp1), .max(maxscore));
//         end
//         // 9: 
...
always @ (*) begin
    case(1)
        // ((A_2_cnt[A]==3)&& trips!=A ) : trips_2 = A;
        ((A_2_cnt[K]==3)&& trips!=K ) : trips_2 = K;
        ((A_2_cnt[Q]==3)&& trips!=Q ) : trips_2 = Q;
        ((A_2_cnt[J]==3)&& trips!=J ) : trips_2 = J;
        ((A_2_cnt[10]==3)&& trips!=10): trips_2 = 4'd10;
        ((A_2_cnt[9]==3)&& trips!=9 ) : trips_2 = 4'd9;
        ((A_2_cnt[8]==3)&& trips!=8 ) : trips_2 = 4'd8;
        ((A_2_cnt[7]==3)&& trips!=7 ) : trips_2 = 4'd7;
        ((A_2_cnt[6]==3)&& trips!=6 ) : trips_2 = 4'd6;
        ((A_2_cnt[5]==3)&& trips!=5 ) : trips_2 = 4'd5;
        ((A_2_cnt[4]==3)&& trips!=4 ) : trips_2 = 4'd4;
        ((A_2_cnt[3]==3)&& trips!=3 ) : trips_2 = 4'd3;
        ((A_2_cnt[2]==3)&& trips!=2 ) : trips_2 = 4'd2;
        default : trips_2 = 4'b0;
    endcase
end
...
// assign flush_cnt = flush_mask[2] + flush_mask[3] + flush_mask[4] + flush_mask[5] + flush_mask[6] +
//                    flush_mask[7] + flush_mask[8] + flush_mask[9] + flush_mask[10] +
//                    flush_mask[J] + flush_mask[Q] + flush_mask[K] + flush_mask[A];

reg [3:0] findmax_straight_flush;

always @(*) begin
    // if(flush_cnt>=5)
        case(1)  
            // (flush_mask[10] && flush_mask[J] && flush_mask[Q] && flush_mask[K] && flush_mask[A]) : findmax_straight_flush = A;
            (flush_mask[9]  && flush_mask[10]&& flush_mask[J] && flush_mask[Q] && flush_mask[K]) : findmax_straight_flush = K;
            (flush_mask[8]  && flush_mask[9] && flush_mask[10]&& flush_mask[J] && flush_mask[Q]) : findmax_straight_flush = Q;
            (flush_mask[7]  && flush_mask[8] && flush_mask[9] && flush_mask[10]&& flush_mask[J]) : findmax_straight_flush = J;
            (flush_mask[6]  && flush_mask[7] && flush_mask[8] && flush_mask[9] && flush_mask[10]): findmax_straight_flush = 4'd10;
            (flush_mask[5]  && flush_mask[6] && flush_mask[7] && flush_mask[8] && flush_mask[9]) : findmax_straight_flush = 4'd9;
            (flush_mask[4]  && flush_mask[5] && flush_mask[6] && flush_mask[7] && flush_mask[8]) : findmax_straight_flush = 4'd8;
            (flush_mask[3]  && flush_mask[4] && flush_mask[5] && flush_mask[6] && flush_mask[7]) : findmax_straight_flush = 4'd7;
            (flush_mask[2]  && flush_mask[3] && flush_mask[4] && flush_mask[5] && flush_mask[6]) : findmax_straight_flush = 4'd6;
            (flush_mask[A]  && flush_mask[2] && flush_mask[3] && flush_mask[4] && flush_mask[5]) : findmax_straight_flush = 4'd5;
            default : findmax_straight_flush = 0;  
        endcase
    // else
        // findmax_straight_flush = 0;
end
...
always @(*) begin
    case(1)
        (straight == A && flush_mask[A] && flush_mask[K] && flush_mask[Q] && flush_mask[J] && flush_mask[10]) : score = {4'd10, 4'd0 ,4'd0,4'd0,4'd0,4'd0}; // Royal Flush
        ...
        (findmax_straight_flush>0) : score = {4'd9, findmax_straight_flush ,4'd0,4'd0,4'd0,4'd0}; // Straight Flush
        (quads >0) : begin
            case (1)
        ...
        (trips >0 && ((pairs >0)|| (trips_2 >0) ) && quads==0 ) : begin
            if (trips_2 ==0 )
                score = {4'd7, trips ,pairs ,4'd0,4'd0,4'd0}; // Full House
            else
                score = {4'd7, trips ,trips_2 ,4'd0,4'd0,4'd0}; // Full House XXX YYY situation
        end
        // (trips >0 && trips_2 >0 && quads==0 ) : score = {4'd7, trips ,trips_2 ,4'd0,4'd0,4'd0}; // Full House XXX YYY situation
        (has_flush) : begin
            case(1)
        ...
        (straight >0) : 
            score = {4'd5, straight ,4'd0,4'd0,4'd0,4'd0}; // Straight
        ( trips>0 ) : begin
            // minor
            case (1)
...
// remove flush_cnt
// Area: 235694.289830    WinRate : 574313.241724 -> 547883.683589 (VVVVVV)

//let royal flush score ={10,0,0,0,0,0}
// Area: 235450.555449   WinRate : 547883.683589 -> 552074.039989 (XXXXXX)

//remove // if(has_flush) (v305)
// Area: 232769.477112   WinRate : 552074.039989 -> 541346.601776

//remove findmax module
// Area: 246765.456821(but CT reduce)   WinRate : 415426.536132 -> 377557.086442
