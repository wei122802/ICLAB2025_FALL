...
always @ (posedge clk or negedge rst_n) begin
    ...
    else if ((state_c == PREDICT) && cnt>22) begin
...
always @ (posedge clk or negedge rst_n) begin
    ...
    else if (state_c == PREDICT) cnt <= cnt + 1;
    else cnt <= cnt;
end
...
always @(posedge clk or negedge rst_n) begin
    ...
    else if ((state_c == PREDICT) && cnt<52) begin
        if(!exist_card)
...
always @(*) begin
    if((state_c == PREDICT)&&(cnt<489)) predict_num2 = _31_card_num[predict2_cnt];
    else predict_num2 = 0;
end

always @(*) begin
    if((state_c == PREDICT)&&(cnt<489)) predict_suit2 = _31_card_suit[predict2_cnt];
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        for(i = 0; i < 9; i = i + 1)
            player_score[i] <= 15'd0;
    end else if((state_c == PREDICT) && cnt>23) begin
...
// always @(*) begin
//     div_temp = player_score[div_cnt];
// end

assign player_temp = (player_score[div_cnt] * 10) / 1674;
...
always @ (posedge clk or negedge rst_n) begin
    ...
    else if (state_c == OUTPUT) out_win_rate <= out_win_rate_comb;
...
// Cycle: 13.00  
// Area: 479906.784067
// Performance: 6238788.19287100 *53000 = 330482908756.6300

// Cycle: 13.00  *******************3 *52900
// Cycle: 13.00
// Area: 485209.569660
// Performance: 6307724.40558000 *52900 = 333798498754.8200

// Cycle: 13.00 ***************3
// Cycle: 13.00
// Area: 475857.043137
// Performance: 6186141.56078100 *50000 = 309307078050.5000

//change FSM number 2
// Cycle: 13.00
// Area: 487203.191794
// Performance: 6333641.49332200

// 4 FSM no change remove predict1_cnt rst_n
// Cycle: 13.00
// Area: 484062.767984
// Performance: 6292815.98379200

// Cycle: 13.00 ****
// Area: 470501.136064
// Performance: 6116514.76883200 *50000 = 305825738441.6000

// Cycle: 13.00
// Area: 467516.952015
// Performance: 6077720.37619500 *50000 = 303886018809.7500

// Cycle: 13.00 /16740 -> /1674 **********
// Area: 466248.283462
// Performance: 6061227.68500600 *50000 = 303061384250.3000

// Cycle: 13.00 deal_num case ***********
// Area: 461804.817703
// Performance: 6003462.63013900 *50000 = 300173131506.9500
